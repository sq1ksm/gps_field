...
//
// Modyfikacje:
// - 26-02-2016   V.1.0   Pierwsze wydanie
// - 01-11-2016   V.1.1   Zmieniono wyświetlanie czasu lokalnego w oryginalnym Adafruit_GPS.cpp
// - 17-04-2021   V.2.0   Dodano ustawienia strefy czasowej i formatu 12/24 godzinnego. Pobrano z clock_sevenseg_gps.ino
// - 05-03-2023   V.2.1   Naprawiono problem z określaniem kierunków wschód/zachód
// - 25-11-2024   V.2.2   Naprawiono błędne obliczenia lokalizatora QRA przy ujemnej długości/szerokości geograficznej
...
// Ustaw na 'true', jeśli chcesz debugować i słuchać surowych zdań GPS. 
...
GPS.sendCommand(PMTK_SET_NMEA_OUTPUT_RMCGGA); // zmniejsza do niezbędnego minimum wysyłane dane - nowa funkcja! (SQ1KSM)
  // Przypisz domyślną wartość koloru
  if ( u8g.getMode() == U8G_MODE_R3G3B2 ) {
    u8g.setColorIndex(255);     // biały
  }
  else if ( u8g.getMode() == U8G_MODE_GRAY2BIT ) {
    u8g.setColorIndex(3);       // maksymalna intensywność
  }
  else if ( u8g.getMode() == U8G_MODE_BW ) {
    u8g.setColorIndex(1);       // piksel włączony
  }
...
void displaylocation()
{
  ...
  if (GPS.satellites == 0) {
    ...
    latns = GPS.lat;
    lonew = GPS.lon;
    u8g.print(GPS.latitudeDegrees); u8g.print(latns);
    u8g.print(" - "); 
    u8g.print(GPS.longitudeDegrees); u8g.print(lonew);  
  }
}
...
void displayQRA()
{
  ...
  if (lonew == 'W') 
  {
    loclong = 180000000 - (abs(loclong));   // Wersja 2.2
  }
  ...
  if (latns == 'S') 
  {
    loclat = 90000000 - (abs(loclat));    // Wersja 2.2
  }
...
void displaysats()
{
  ...
  if (GPS.satellites >= 0 && GPS.satellites < 20)  {
    u8g.print("Satelit: "); u8g.print((int)GPS.satellites);
  }  
}
